...
export const ChemicalBondHeader: React.FC<HeaderProps> = ({
  navItems,
  activeSection,
  setActiveSection
}) => {
  const quickNavTags: QuickNavTag[] = [
    { targetId: 'section-covalent', label: 'Ковалентная связь', icon: Atom },
    { targetId: 'section-crystal-lattices', label: 'Кристаллические решётки', icon: TestTube },
    { targetId: 'section-intermolecular', label: 'Межмолекулярное взаимодействие', icon: Orbit },
  ];
  const { openStudyBlock } = useRouter();

  const handleGoToPractice = () => {
    openStudyBlock('general-chemistry', 'gen-chem-bond', 'practice');
  };
    ...
    <div className="space-y-8 font-body text-slate-800 leading-relaxed text-sm sm:text-base">
      ...
      <div className="bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-wrap items-center gap-2 text-xs sm:text-sm text-slate-600 font-medium">
          ...
          <span>•</span>
          <span>Раздел 3 • Химическая связь</span>
        </div>

        <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 tracking-tight flex items-center gap-3">
          <FlaskConical className="w-8 h-8 text-slate-800 shrink-0" />
          <span>Химическая связь и строение вещества</span>
        </h1>

        <p className="text-sm sm:text-base text-slate-700 leading-relaxed font-normal max-w-4xl">
          Валентность и степень окисления, ковалентная связь и геометрия молекул, ионная и металлическая связь, межмолекулярные взаимодействия и типы кристаллических решёток.
        </p>
      ...
      <div className="p-4 sm:p-5 rounded-xl bg-slate-50 border border-slate-200 text-slate-800 flex items-start gap-3.5 shadow-xs">
        ...
        <div className="space-y-1 text-xs sm:text-sm">
          ...
          <p className="text-slate-600 leading-relaxed font-normal">
            Тип химической связи и тип кристаллической решётки определяют свойства вещества — от температур плавления и кипения до электропроводности. Валентность не имеет знака, а степень окисления имеет.
          </p>
        </div>
      </div>
      ...
      <div className="p-5 rounded-xl bg-slate-50 border-l-4 border-l-amber-500 border-y border-r border-slate-200 text-slate-800 space-y-3 shadow-xs">
        ...
        <ul className="space-y-2.5 text-xs sm:text-sm text-slate-700 font-normal leading-relaxed">
          <li className="flex items-start gap-2">
            ...
            <div>
              <strong>Степень окисления и валентность в простых веществах</strong>: степень окисления в простых веществах равна нулю, а валентность в них не равна нулю: например, валентность азота в <ChemFormula formula="N2" /> равна III.
            </div>
          </li>
          <li className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-slate-200 text-slate-900 text-xs font-mono font-bold shrink-0">2</span>
            <div>
              <strong>Донорно-акцепторный механизм</strong>: в ионах <ChemFormula formula="NH4+" /> и <ChemFormula formula="H3O+" /> связь образуется по донорно-акцепторному механизму, но все четыре связи в ионе аммония абсолютно эквивалентны.
            </div>
          </li>
          <li className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-slate-200 text-slate-900 text-xs font-mono font-bold shrink-0">3</span>
            <div>
              <strong>Водородная связь</strong>: примерно в 10 раз слабее ковалентной, но именно она объясняет аномально высокие температуры кипения воды, аммиака и фтороводорода.
            </div>
          </li>
          <li className="flex items-start gap-2">
            <span className="px-1.5 py-0.5 rounded bg-slate-200 text-slate-900 text-xs font-mono font-bold shrink-0">4</span>
            <div>
              <strong>Тип решётки определяет свойства</strong>: вещества с молекулярной решёткой летучи и легкоплавки, а с атомной решёткой (алмаз, <ChemFormula formula="SiO2" />) — исключительно твёрдые и тугоплавкие.
            </div>
          </li>
        </ul>
      </div>
