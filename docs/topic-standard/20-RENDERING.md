...
- **Компактное превью читаемо — без «микромира» в тексте статьи**: `MolecularDiagramBody` принимает `compact={!isModal}` — подписи атомов укрупняются до читаемого минимума (16 SVG-единиц), а размерные линии, дуги углов и пояснительные надписи скрываются (все детали рассматриваются в модалке); `SvgDiagramWrapper` в компактном режиме автоподбирает viewBox под габариты контента. Запрещено оставлять в превью-плитке текст, который на экране стал бы мельче ~10px. Полные правила превью — §2.4.
- **Несколько структурных формул в одном блоке — светлая панель на белом фоне** (полный формат — §2.4): если раздел показывает сразу несколько структурных формул (ряд молекул, семейство кислот), НЕ трамбовать их в тёмную компактную плитку — строить светлую академическую панель. Эталоны: фосфорные кислоты в N/P ([`PhosphorusAcids2DRenders.tsx`](../../src/components/study/topics/nitrogenPhosphorus/PhosphorusAcids2DRenders.tsx)) и панель X₂ в галогенах (`HalogensX2LightPanel` в [`Halogens2DRenders.tsx`](../../src/components/study/topics/halogens/Halogens2DRenders.tsx)).
