...
Разделы с плотной сводной информацией (разложение солей по типу кислоты, зависимость продуктов ОВР от среды pH, аллотропия, цепочки превращений) дополняются **tree-диаграммой** — по образцу схемы термического разложения солей аммония в теме N/P. Схема не заменяет текст: секция сохраняет вводный абзац (§3 30-DESIGN) и пояснения к ветвям.

**Паттерн**: светлая панель `p-6 bg-slate-50/70 rounded-2xl border border-slate-200 shadow-sm space-y-4` с шапкой (название процесса справа, `text-xs text-slate-500`); слева «корневой» объект (например, «Соли аммония»); в центре — SVG-ветвления со стрелками (`marker`-наконечники, линии `text-slate-400 stroke-[1.75]`) и подписями ветвей в плашках (`bg-slate-50 border border-slate-200/80 rounded-md text-xs`); справа — сгруппированные реакции по ветвям, все формулы через `ChemFormula`. Горизонтальная прокрутка для узких экранов: `overflow-x-auto` + `min-w-[720px]`.

**Правила**:
- **Нумерация схем запрещена**: заголовки вида «Схема 1», «Схема 11» не пишутся — заголовок схемы является названием процесса; номер несёт лишнюю информацию.
- `font-serif` запрещён (§1 30-DESIGN) — заголовки схемы набираются `font-body`/`font-heading`.
- Язык — 100% русский (§2.5); цвета — в границах палитры 30-DESIGN §2.
- Каждая ветвь подписана условием/классом, реакции внутри ветви — от частных случаев к общим.

### 2.7. Концептуальная инфографика — `InfographicFigure`

Концептуальные (НЕмолекулярные) схемы — классификации (деревья), количественные законы (бар-чарты), мини-графики зависимостей — строятся **только** централизованным компонентом [`InfographicFigure`](../../src/components/scientific/svg/InfographicFigure.tsx). **Запрещено** собирать концептуальные схемы на `SvgDiagramWrapper`/молекулярных примитивах (§2.2) и в плиточно-модальном паттерне (§2.4 — только для молекулярных структурных формул).

**Когда применять**: только там, где схема реально помогает пониманию (классификация, целочисленные отношения, «хабы» формул); инфографика ради украшения не добавляется.

**Современный формат учебной фигуры** (эталон — ОХ-01, [`GeneralBasicsInfographics.tsx`](../../src/components/study/topics/generalBasics/GeneralBasicsInfographics.tsx)):

- **Светлая академическая тема**: токены `LIGHT_PALETTE` + 8 цветовых категорий `InfoCategory` ([`molecule2DTheme.ts`](../../src/utils/molecule2DTheme.ts)) — та же палитра, что в светлом варианте генератора 2D-структурных формул; цветные рамки/тонировки блоков, легенда, цветные бары.
- **Встраивание в секцию без модальных окон**: заголовок фигуры сверху; схема; справочная панель с янтарными акцентами **справа** (на узких экранах — под схемой); подпись-ключевой вывод снизу (допускаются `ChemFormula`/`ChemText`). Справочные детали и примеры живут в панели/подписи, а не в блоках схемы.
- **Компактность**: блоки по возможности однострочные; масштаб схемы ограничен рассчитанными габаритами (не раздувается на всю ширину, пустых полей нет); текст читаем без «микромира».
- **Дата-драйвен**: тема описывает схему спеком (`nodes` / `edges` / `bars` / `plots` / `notes`); размеры блоков вычисляются по канвас-измерению текста ([`measureSvgText.ts`](../../src/utils/measureSvgText.ts) — точная метрика кириллицы), связи обрезаются у рассчитанных границ — наползания исключены по построению.
- Язык — 100 % русский (§2.5); нумерация схем запрещена (§2.6).

### 2.8. Интерактивные концепт-карты — `ConceptFlow` (React Flow)

Концепт-карты и формульные «хабы», где интерактивность несёт учебную пользу (зум, панорамирование, клик по узлу с раскрытием формулы и примера расчёта), строятся централизованным компонентом [`ConceptFlow`](../../src/components/interactive/ConceptFlow.tsx) на зависимости `@xyflow/react` (React Flow). Статические фигуры без интерактива остаются за `InfographicFigure` (§2.7). Эталон — моль-хаб ОХ-01 ([`GeneralBasicsConceptFlow.tsx`](../../src/components/study/topics/generalBasics/GeneralBasicsConceptFlow.tsx)).

- Тема описывает карту данными (`nodes` / `edges` / `details` / `reference`); координаты рассчитываются автораскладкой «хаб + спутники», ручная разметка не нужна.
- Связи: основные переходы — сплошные двунаправленные стрелки с формулами; вторичные соотношения — тонкие пунктирные дуги по периметру раскладки (флаг `ring`).
- **Правило непопадания подписей**: подписи связей не должны накладываться на блоки. В спеках подписи держат короткими (формулы без единиц — единицы и примеры в `details`/справочной панели); компонент автоматически смещает подпись шире свободного зазора над линией.
- Палитра и цветовые категории — единые с §2.7; клик по узлу раскрывает `details` (допускаются `ChemFormula`); у каждого узла с интерактивом обязано быть содержимое `details` (хаб — не исключение).
