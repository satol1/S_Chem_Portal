...
- Dark-блоки 2–5 шт.: 2–3 больших + 1–2 малых; содержание не ограничено промышленным химизмом ([`../30-DESIGN.md`](../30-DESIGN.md) §6). `TheoryCallout` ≥ 2 шт., равномерно по секциям ([`../30-DESIGN.md`](../30-DESIGN.md) §5).
- Каждая секция начинается вводным абзацем теории ([`../30-DESIGN.md`](../30-DESIGN.md) §3); плотная сводная информация дополняется tree-диаграммой без номера ([`../20-RENDERING.md`](../20-RENDERING.md) §2.6); ≥ 3 структурных формул в разделе — светлая панель ([`../20-RENDERING.md`](../20-RENDERING.md) §2.4).
